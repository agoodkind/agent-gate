...
func TestStoreConfigurePrunesOversizedEntries(t *testing.T) {
	store := New(Options{MaxValueBytes: 8, PruneInterval: 0})
	defer store.Close()

	_, _, err := store.Set("exec", "repo", []byte("12345678"), SetOptions{})
	if err != nil {
		t.Fatalf("Set: %v", err)
	}

	store.Configure(Options{MaxValueBytes: 4, PruneInterval: 0})
	...
	if found {
		t.Fatal("Get found = true, want oversized entry pruned after Configure")
	}
}
